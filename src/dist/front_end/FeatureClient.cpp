...
// from the server.
bool FeatureClient::onQuery(const cv::Mat &image, const CameraModel &camera,
  ...
  std::vector<uchar> imageStr;
  imencode(".jpg", image, imageStr);
  query.set_image(imageStr.data(), imageStr.size());
