...
const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &Image::getCloud(int decimation) const {
  //generate a new pcl point cloud and return
  return removeNaNFromPointCloud(cloudFromDepthRGB(_image, _depth, _camera, decimation, 0, 0));
}




pcl::PointCloud<pcl::PointXYZRGB>::Ptr Image::cloudFromDepthRGB(
		...
		bool estWithNeighborsIfNull) const
{
	...
	{
		fprintf( stderr,"!(x >=0 && x<depthImage.cols && y >=0 && y<depthImage.rows) cond failed! returning bad point. (x=%f (u=%d), y=%f (v=%d), cols=%d, rows=%d)",
...
pcl::PointXYZ Image::projectDepthTo3D (
		const cv::Mat & depthImage,
		float x, float y,
		float cx, float cy,
		float fx, float fy,
