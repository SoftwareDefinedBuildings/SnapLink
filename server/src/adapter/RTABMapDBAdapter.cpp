...
bool RTABMapDBAdapter::readData(const std::vector<std::string> &dbPaths,
  ...
  std::map<int, std::list<std::unique_ptr<Signature>>> allSignaturesMap;
  ...
  for (const auto &dbPath : dbPaths) {
    ...
    allSignaturesMap.insert(std::make_pair(dbId, std::move(dbSignatures)));
  ...
  auto mergeSignaturesIdMap = getMergeSignaturesIdMap(allSignaturesMap);

  auto mergedSignatures = mergeSignatures(
      std::move(allSignaturesMap), mergeSignaturesIdMap, mergeWordsIdMap);
  signatures.putSignatures(std::move(mergedSignatures));

  auto mergedWords =
      mergeWords(std::move(allWordsMap), mergeWordsIdMap, mergeSignaturesIdMap);
...
std::list<std::unique_ptr<Signature>>
RTABMapDBAdapter::readSignatures(const std::string &dbPath, int dbId) {
  ...
  for (auto &signature : rtabmapSignatures) {
    ...
    signatures.emplace_back(std::unique_ptr<Signature>(
        new Signature(id, mapId, dbId, std::move(pose),
  ...
  if (rc == SQLITE_OK) {
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      ...
      if (getPoint3World(signatures, dbId, imageId, x, y, pWorld)) {
        ...
        UINFO("Read point (%lf,%lf,%lf) with label %s in database %s", pWorld.x,
              pWorld.y, pWorld.z, name.c_str(), dbPath.c_str());
      }
    }
  } else {
...
bool RTABMapDBAdapter::getPoint3World(
    const std::list<std::unique_ptr<Signature>> &signatures, int dbId,
    int imageId, int x, int y, pcl::PointXYZ &pWorld) {
  UDEBUG("");
...
std::map<std::pair<int, int>, int> RTABMapDBAdapter::getMergeSignaturesIdMap(
    const std::map<int, std::list<std::unique_ptr<Signature>>> &signaturesMap) {
  std::map<std::pair<int, int>, int> mergeSignaturesIdMap;
  int nextSignatureId = 1;
  for (const auto &signatures : signaturesMap) {
    int dbId = signatures.first;
...
std::list<std::unique_ptr<Word>> RTABMapDBAdapter::mergeWords(
    std::map<int, std::list<std::unique_ptr<Word>>> &&wordsMap,
