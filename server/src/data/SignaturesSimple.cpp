...
void SignaturesSimple::putSignatures(
...
const std::map<int, std::unique_ptr<Signature>> &
SignaturesSimple::getSignatures() const {
  return _signatures;
}

std::vector<int> SignaturesSimple::findKNN(const std::vector<int> &wordIds,
                                           int k) const {
  ...
  if (similarities.size()) {
    ...
    std::partial_sort_copy(similarities.begin(), similarities.end(),
                           topSimilarities.begin(), topSimilarities.end(),
                           compareSimilarity);
...
float SignaturesSimple::computeSimilarity(const std::vector<int> &wordIds,
...
bool SignaturesSimple::compareSimilarity(const std::pair<int, float> &l,
