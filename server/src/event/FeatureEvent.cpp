...
FeatureEvent::FeatureEvent(std::unique_ptr<std::vector<cv::KeyPoint>> &&keyPoints,
               std::unique_ptr<cv::Mat> &&descriptors,
               std::unique_ptr<CameraModel> &&camera,
                           std::unique_ptr<PerfData> &&perfData,
                           const void *session)
    : QEvent(FeatureEvent::type()), _keyPoints(std::move(keyPoints)), _descriptors(std::move(descriptors)),_camera(std::move(camera)),
