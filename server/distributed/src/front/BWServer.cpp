...
const std::string BWServer::busypage =
    "This server is busy, please try again later.";
const std::string BWServer::errorpage = "This doesn't seem to be right.";

BWServer::BWServer() : _daemon(nullptr), _numClients(0) {}
...
bool BWServer::init(std::string featureServerAddr, uint16_t port,
...
grpc::Status BWServer::onDetection(grpc::ServerContext *context,
...
void BWServer::run(std::string bwServerAddr) {
  std::string server_address(bwServerAddr);
...
int BWServer::answerConnection(void *cls, struct MHD_Connection *connection,
  ...
  BWServer *bwServer = static_cast<BWServer *>(cls);
  assert(bwServer != nullptr);

  if (*con_cls == nullptr) // new connection
  {
    if (bwServer->getNumClients() >= bwServer->getMaxClients()) {
    ...
    connInfo->session->id = bwServer->_dis(bwServer->_gen);
    connInfo->session->type = HTTP_POST;

    bwServer->_mutex.lock();
    bwServer->_connInfoMap.insert(
        std::make_pair(connInfo->session->id, connInfo));
    bwServer->_mutex.unlock();
    ...
    bwServer->setNumClients(bwServer->getNumClients() + 1);
  ...
  if (*upload_data_size != 0) {
    ...
    *upload_data_size = 0;
    ...
    if (!connInfo->rawData->empty()) {
      ...
      FeatureClient client(bwServer->_channel);
...
int BWServer::iteratePost(void *coninfo_cls, enum MHD_ValueKind kind,
...
void BWServer::requestCompleted(void *cls, struct MHD_Connection *connection,
                                  void **con_cls,
  ...
  bwServer->_mutex.lock();
  bwServer->_connInfoMap.erase(session->id);
  bwServer->_mutex.unlock();
...
int BWServer::sendPage(struct MHD_Connection *connection,
